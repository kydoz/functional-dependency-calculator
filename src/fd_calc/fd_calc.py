...
class FDCalc:
    ...
    def calc_fds(self):
        for i in range(1, self.nb_atts):
            ...
            res = [comb for comb in combinations(self.attributes, i)]

            res_unique = self.calc_unique_fds(res)
    ...
    def calc_final_dets(self, dets_collection):
    ...
    def calc_repeating_fds(self, res, res_unique):
        for temp in res:
            calc = {}
            previously_seen = {}
            str_temp = str(temp)
                ...
                for vals in calc[left_side]:
                    ...
                    if len(dets) == 0:
                        ...
                        if key_temp not in seen:
                            ...
                            seen[key_temp] = ""
                if not was_empty:
                    res_dets = self.calc_final_dets(dets_collection)
                    if len(res_dets) > 0:
